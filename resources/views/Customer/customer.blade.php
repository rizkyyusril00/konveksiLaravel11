<x-layout>
    <div class="w-full md:w-[75%] lg:w-[80%] pt-4 px-3 md:pl-12 md:pr-4 lg:px-4 h-screen bg-primary flex flex-col gap-4">
        ...
        {{-- header --}}
        <x-header title="Customer" addRoute="AddCustomer" />
        ...
        <div class="hidden md:flex items-center justify-between gap-2">
            ...
            <a href="/add/customer" class="btn h-10 min-h-10 btn-secondary w-fit flex items-center gap-2 group">
                ...
                <span class="text-[12px] text-primary">Tambah Customer</span>
            </a>
        </div>

        {{-- action mobile --}}
        <form method="GET" action="/customer" class="md:hidden flex items-center gap-2">
            {{-- search --}}
            <div class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama..."
                    class="input input-bordered input-sm input-secondary pr-10 w-40 text-[14px]" />
            ...
            <a href="/customer" class="flex items-center justify-center bg-secondary w-[30px] h-[30px] rounded-full">
                <svg xmlns="http://www.w3.org/2000/svg"
                    class="hover:scale-[1.03] transition-all duration-300 ease-in-out" width="18" height="18"
                    fill="#ffffff" viewBox="0 0 256 256">
                    <path
                                            ...
                                            <template x-if="open">
                                                <div x-transition:enter="transition transform ease-out duration-300"
                                                    ...
                                                    <div class="bg-white p-6 rounded-lg shadow-lg w-fit">
                                                        <h3 class="text-lg font-bold">Hapus Customer Ini?</h3>
